...
# Função para pegar API key do Eleven Labs
set_api_key(st.secrets["ELEVEN_LABS_API_KEY"])
...
st.sidebar.title("Assistente Speck EAD")
st.sidebar.divider()
st.sidebar.markdown("Developed by Emídio Souza")
...
def generate_audio_from_text(tts_text):
    ...
    voice=Voice(
        voice_id='d6sWmVTrFWLo3jOTmOt7',
        settings=VoiceSettings(stability=0.5, similarity_boost=0.8, style=0.8, use_speaker_boost=True)
        )
    )
    return audio
 ...
 # Iterando nas mensagens para encontrar a última mensagem do bot
    if st.session_state.messages.data:
        ...
        if ultima_mensagem and ultima_mensagem.content: # Verificando se a última mensagem do assistente foi encontrada
                audio = generate_audio_from_text(ultima_mensagem.content[0].text.value) # Acessa o texto da mensagem e gera áudio do texto    
